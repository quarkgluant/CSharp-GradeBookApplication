...
namespace GradeBook.UserInterfaces
{
    public static class StartingUserInterface
    {
        ...
        public static void CreateCommand(string command)
        {
            ...
            GradeBookType typeValue;

            if (Enum.TryParse(type, true, out typeValue))
            {
                switch (typeValue)
                {
                    case GradeBookType.Standard:
                        var standardGradeBook = new StandardGradeBook(name, isWeighted);
                        Console.WriteLine("Created gradebook {0}.", name);
                        GradeBookUserInterface.CommandLoop(standardGradeBook);
                        break;
                    case GradeBookType.Ranked:
                        var rankedGradeBook = new RankedGradeBook(name, isWeighted);
                        Console.WriteLine("Created gradebook {0}.", name);
